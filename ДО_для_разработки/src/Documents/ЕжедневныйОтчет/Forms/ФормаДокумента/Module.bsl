...
//бит_2018.12.28 рм+
&НаСервере
Функция КорректноЗаполненыПроекты()
	ПроектныеРаботы = Справочники.ВидыРабот.НайтиПоКоду("00-000003");
	Для каждого стр из Работы Цикл
		// Чтобы не забывали указывать проектную задачу во всех работах, а то иногда выбирают не тот Вид работ
		//Если стр.ВидРабот.ПринадлежитЭлементу(ПроектныеРаботы)
		//	и Не ЗначениеЗаполнено(стр.ПроектнаяЗадача) Тогда
		// стр.ВидРабот.ПринадлежитЭлементу(ПроектныеРаботы)
		Если Не ЗначениеЗаполнено(стр.ПроектнаяЗадача) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//бит_2018.12.28 рм+
	Если Не КорректноЗаполненыПроекты() Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
	//	Сообщение.Текст = "По проектным работам обязательно должна быть указана проектная задача. ЕЖЕДНЕВНЫЙ ОТЧЕТ НЕ ЗАПИСАН!";   - чтобы не забывали указывать задачи
		Сообщение.Текст = "Обязательно должна быть указана проектная задача. ЕЖЕДНЕВНЫЙ ОТЧЕТ НЕ ЗАПИСАН!";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;	
	//бит_2018.12.28 рм-
